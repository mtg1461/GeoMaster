...
function shouldUseCompactFx() {
  return prefersReducedMotion() || Math.min(window.innerWidth, window.innerHeight) <= 700;
}
...
function playCorrectFx(container) {
  ...
  const compactFx = shouldUseCompactFx();

  if (typeof window.confetti === "function" && !prefersReducedMotion()) {
    window.confetti({
      position: { x: window.innerWidth / 2, y: window.innerHeight / 2 },
      count: compactFx ? 34 : 70,
      size: compactFx ? 0.92 : 1.15,
      velocity: compactFx ? 150 : 190,
      fade: true,
    });
  ...
  const pieceCount = compactFx ? 12 : 24;
