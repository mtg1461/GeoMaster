...
const { difficultyBuckets } = window.GeoMaster.countries;
...
const bucketNames = ["diff1", "diff2", "diff3", "diff4", "diff5"];
...
class GeoGame {
  ...
  buildRounds() {
    const recipe = difficultyRecipes[this.options.difficulty] || difficultyRecipes.normal;
    const unit = Math.max(1, Math.floor(this.options.count / 5));
    const selected = recipe.flatMap((multiplier, index) => {
      const bucket = difficultyBuckets[bucketNames[index]];
  ...
  submitGuess() {
    ...
    const guess = normalizeAnswer(this.elements.guess.value);
    const answer = normalizeAnswer(this.currentCountry);
