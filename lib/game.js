...
class GeoGame {
  ...
  startRoundTimer(duration = this.getRoundTimeLimit()) {
    ...
    this.roundTimerRemaining = duration;
    this.roundTimerEndTime = performance.now() + (duration * 1000);

    const updateTimer = () => {
      const secondsLeft = Math.max(0, Math.ceil((this.roundTimerEndTime - performance.now()) / 1000));
      const progress = (secondsLeft / duration) * 360;
      ...
      this.elements.roundTimerStat.style.setProperty("--timer-progress", `${progress}deg`);
