...
function renderHint(container, country, revealedIndexes) {
  ...
  if (container.dataset.country === country && container.children.length === letters.length) {
    letters.forEach((letter, index) => {
      if (/[a-z]/i.test(letter)) {
        container.children[index].textContent = revealedIndexes.has(index) ? letter.toUpperCase() : "";
      }
    });
    return;
  }
...
class GeoGame {
  ...
  setRoundResolving(isResolving) {
    this.isRoundResolving = isResolving;

    if (this.elements.skip) {
      this.elements.skip.disabled = isResolving;
    }
  }
  ...
  nextRound() {
    ...
    const zoomPromise = waitForTransitionPrep().then(() => {
      ...
      this.isTransitionPreparing = false;
      this.elements.gamePanel.classList.add("is-camera-moving");
      return this.map.zoomToPreparedCountry(preparedTransition);
    });

    Promise.resolve(zoomPromise).then((target) => {
      this.isTransitionPreparing = false;
      this.elements.gamePanel.classList.remove("is-camera-moving");
  ...
  submitGuess() {
    if (!this.currentCountry || this.isRoundResolving) {
      return;
    }
  ...
  selectChoice(choice, button) {
    ...
    if (!this.isCorrectAnswer(choice)) {
      ...
      this.revealCorrectChoice(button, "wrong");
      window.GeoMaster.soundEffects?.play("wrongAnswer");
      playWrongFx(this.elements.feedbackFx);
      ...
      this.stopTimer();
      this.roundToken += 1;
      this.nextRoundTimeout = window.setTimeout(() => this.nextRound(), this.getResolutionDelay(760));
      return;
    }

    button?.classList.add("is-correct");

    if (this.getCurrentMode().revealChoiceLabelsAfterAnswer) {
      this.revealCorrectChoice(button, "correct");
    }
  ...
  selectMapCountry(country) {
    if (!this.currentCountry || !country || this.isRoundResolving) {
      return;
    }
  ...
  skip() {
    ...
    if (mode.revealAnswerOnSkip) {
      this.revealCorrectChoice(null, "skip");
    }

    flashPanel(this.elements.guessPanel, "flash-timeout");
    this.nextRoundTimeout = window.setTimeout(() => this.nextRound(), this.getResolutionDelay(650));
  }
  ...
  getResolutionDelay(defaultDelay) {
    return this.getCurrentMode().resolutionDelayMs ?? defaultDelay;
  }
  ...
  completeRound() {
    ...
    this.nextRoundTimeout = window.setTimeout(() => this.nextRound(), this.getResolutionDelay(700));
  }
  ...
  clearRoundInput() {
    ...
    delete this.elements.hint.dataset.country;
  ...
  revealCorrectChoice(selectedButton = null, outcome = "answer") {
  ...
  startRoundTimer(duration = this.getRoundTimeLimit()) {
    ...
    const updateTimer = () => {
      ...
      const nextBoundaryDelay = remainingMs - ((secondsLeft - 1) * 1000);
      this.roundTimerTimeout = window.setTimeout(updateTimer, Math.max(16, nextBoundaryDelay + 8));
    };
  ...
  stopRoundTimer() {
    if (!this.roundTimerTimeout) {
      return;
    }

    window.clearTimeout(this.roundTimerTimeout);
    this.roundTimerTimeout = 0;
  }
  ...
  handleRoundTimerExpired() {
    if (
      !this.currentCountry
      || this.isPaused
      || this.isRoundResolving
      || !this.getRoundTimeLimit()
    ) {
      return;
    }
    ...
    if (this.getCurrentMode().inputKind === "map-click") {
      ...
      this.revealCorrectChoice(null, "timeout");
    }
    this.roundToken += 1;
    this.setRoundResolving(true);
    flashPanel(this.elements.guessPanel, "flash-timeout");
