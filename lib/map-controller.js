...
class MapController {
  ...
  getCountryViewBox(country, options = {}) {
    ...
    let viewWidth = Math.max(target.box.width * padding, minWidth);
    let viewHeight = Math.max(target.box.height * padding, minHeight);
  ...
  async zoomToCountry(country, options = {}) {
    ...
    const markerPoint = showTinyMarker && target.isTinyTarget ? target.markerPoint : null;
