  ...
  startIdleDrift(baseBox, markerPoint = this.markerPoint) {
    ...
    const drift = (now) => {
      ...
      this.svg.setAttribute("viewBox", formatViewBoxValues(
        baseBox.x + (Math.sin(angle) * amplitudeX),
        baseBox.y + (Math.sin(angle * 2) * amplitudeY),
        baseBox.width,
        baseBox.height,
      ));
  ...
  animateViewBox(toBox, duration, options = {}) {
    ...
    return new Promise((resolve) => {
      ...
      const tick = (now) => {
        ...
        this.svg.setAttribute("viewBox", formatViewBoxValues(
          mix(fromBox.x, toBox.x, progress),
          mix(fromBox.y, toBox.y, progress),
          mix(fromBox.width, toBox.width, progress),
          mix(fromBox.height, toBox.height, progress),
        ));
