...
function appendUniqueCountry(list, seenCountries, country, optionKey) {
  const key = optionKey(country);
...
function buildMultipleChoiceOptions({
  ...
  seed,
  shuffleWithSeed,
}, optionCount = 4, optionKey = normalizeAnswer) {
  const correctKey = normalizeAnswer(country);
  const seenCountries = new Set([optionKey(country)]);
  const distractors = [];
  ...
  candidatePools.forEach((pool, poolIndex) => {
    ...
    shuffleWithSeed(pool, Number(seed) + (roundIndex * 97) + poolIndex).forEach((candidate) => {
      ...
      appendUniqueCountry(distractors, seenCountries, candidate, optionKey);
    });
  });
...
const FLAG_CHOICE_MODE = {
  id: "flag-choice",
  label: "Flag Match",
  description: "Choose the flag that matches the highlighted country.",
  prompt: "Choose the matching flag",
  inputKind: "multiple-choice",
  choicePresentation: "flag",
  icon: "assets/flag-choice.svg",
  primaryActionLabel: "Choose",
  optionCount: 4,
  roundTimeLimit: 10,
  usesHints: false,
  ...
  revealChoiceLabelsAfterAnswer: true,
  buildChoices(context) {
    const getVisualId = (country) => {
      return window.GeoMaster.countryFlags?.getFlag(country)?.visualId || normalizeAnswer(country);
    };

    return buildMultipleChoiceOptions(context, this.optionCount, getVisualId);
  },
  isCorrect({ guess, country }) {
    return normalizeAnswer(guess) === normalizeAnswer(country);
  },
...
const modes = [MULTIPLE_CHOICE_MODE, FLAG_CHOICE_MODE, MAP_CLICK_COUNTRY_MODE, TYPED_COUNTRY_MODE];
