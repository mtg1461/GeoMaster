...
function showMenu() {
  ...
  elements.brandRow.hidden = false;
...
function showModeSetup(modeId = getSelectedMode()) {
  ...
  elements.brandRow.hidden = true;
  elements.setupModeIcon.src = mode.icon;
...
function renderDifficultyDistribution() {
  ...
  if (!selectedDifficulty) {
    elements.difficultyDistribution.hidden = true;
    return;
  }

  elements.difficultyDistribution.hidden = false;
...
elements.modeCards.forEach((card) => {
  card.addEventListener("click", () => {
    showModeSetup(card.dataset.modeId || DEFAULT_GAME_MODE_ID);
  });
});
